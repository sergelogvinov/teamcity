...
COPY --from=docker/buildx-bin:0.12.1 /buildx /usr/local/libexec/docker/cli-plugins/docker-buildx
...
COPY --from=bitnami/kubectl:1.28.6 /opt/bitnami/kubectl/bin/kubectl /usr/local/bin/kubectl
